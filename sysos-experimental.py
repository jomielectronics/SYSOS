...
cmdLIST = []
with open('files.json', 'r') as object:
    files = object.read()
...
#         saved_files = json.load(file_json)
#         data = js_r('num.json')
#     return saved_files

def UpdateFileList():
    with open('files.json') as f_in:
        return json.load(f_in)

def SaveToFileList(arg):
    with open('files.json', 'w') as file_json:
        #DataCache = UpdateFileList() + filesToSave #This is the suspected line of code. Needs to join the two dictionaries into 'DataCache'
        DataCache = UpdateFileList()
        DataCache.update(arg)
        # json.dump(DataCache, file_json)
        print(DataCache)


def View(file):
    if file not in ReadFileList().keys():
        write(colored("File not found in internal cache", Error))
    else:
        typingPrint(ReadFileList())
...
def make(i): #Needs fixed. Appears to make file in internal cache, but not save it to files.json Suspected that it is the SaveToFileList function.
    ...
    try:
        filesToSave = {}
        filen = None
        filen = f'{getArgs(i)[0]}.{getArgs(i)[1].lower()}'                  #Defines the name of the desired file to create
        filesToSave[filen] = list([Directory, getArgs(i)[1].capitalize()])  #Adds to the file list by adding it to a dictionary
        with open(f'{getArgs(i)[0]}.txt', 'w') as f:                        #Creates the actual file
            f.write(f"FILE CREATED AT {time.strftime('%Y-%m-%d %H:%M')}\n")
        print(filesToSave)
    except IndexError:
        files[f'{getArgs(i)[0]}'] = list([Directory, 'Direc'])              #Creates a directory version of the file if the inputted parameters were not specified or unclear
    listContents()
...
while True:
    ...
    elif getFunction(prmt) == Commands[9]:                                                            #View
